...
public:
    ...
    {
        std::vector<std::shared_ptr<BaseWrapper>> snapshot;
        {
            std::scoped_lock lock(m_mutex);
            auto it = m_handlers.find(typeid(T));
            if (it == m_handlers.end()) return;
...
private:
    struct BaseWrapper { size_t id; virtual ~BaseWrapper() = default; };
    template<typename T>
    struct Wrapper : BaseWrapper
    {
        Handler<T> fn;
        Wrapper(size_t i, Handler<T> f) { id = i; fn = std::move(f); }
    };

    mutable std::mutex m_mutex;
    std::unordered_map<std::type_index, std::vector<std::shared_ptr<BaseWrapper>>> m_handlers;
    size_t m_nextId = 0;
};
