...
void ImGUIModule::startup()
{
    ...
    m_inputSub = m_inputModule->OnEvent.subscribe(std::bind(&ImGUIModule::onEvent, this, std::placeholders::_1));
...
void ImGUIModule::removeObject(const std::weak_ptr<GUIObject> &object)
{
    if (const std::shared_ptr<GUIObject> &shared_ptr = object.lock())
    {
        ...
        std::erase_if(m_guiObjects,
                      [objectID](const std::shared_ptr<GUIObject> &object) { return object->getID() == objectID; });
    }
}
