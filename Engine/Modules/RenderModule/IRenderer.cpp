...
namespace RenderModule
{
    IRenderer::IRenderer(std::shared_ptr<Logger::Logger> logger,
                         ...
                         std::shared_ptr<ECSModule::ECSModule> ecsModule) : m_logger(logger),
    ...
    void IRenderer::updateRenderList()
    {
        ...
        world.query<PositionComponent, RotationComponent, CameraComponent>().each(
            [&](PositionComponent& pos, RotationComponent& rot, CameraComponent& cam)
            {
        ...
        world.query<RotationComponent, DirectionalLightComponent>().each(
            [&](RotationComponent& rot, DirectionalLightComponent& lightComponent)
            {
        ...
        world.query<PositionComponent, RotationComponent, ScaleComponent, MeshComponent>().each(
            [=](const flecs::entity& entity, PositionComponent& pos, RotationComponent& rot, ScaleComponent& scale,
                MeshComponent& mesh)
            {
                ...
                if (mesh.meshResource)
                {
                    ...
                    {
                        customShader = ShaderFactory::createOrGetShader(mesh.vertShaderResource.value(),
                                                                        mesh.fragShaderResource.value());
                        m_updateRenderPipelineData
                            .customPass
    ...
    void IRenderer::postInit()
    {
        ...
        std::shared_ptr<RShader> RSHRV = m_resourceManager->load<RShader>(
            "../Resources/Shaders/GLSL/Core/reflection.vert");
        std::shared_ptr<RShader> RSHRF = m_resourceManager->load<RShader>(
        ...
        std::shared_ptr<RShader> TSHRV = m_resourceManager->load<
            RShader>("../Resources/Shaders/GLSL/Core/texture.vert");
        std::shared_ptr<RShader> TSHRF = m_resourceManager->load<
            RShader>("../Resources/Shaders/GLSL/Core/texture.frag");
        ...
        std::shared_ptr<RTexture> DAT = m_resourceManager->load<RTexture>(
            "../Resources/Textures/Generic/DefaultAlbedo.png");
        m_albedoGeneric = TextureFactory::createOrGetTexture(DAT);

        std::shared_ptr<RTexture> DET = m_resourceManager->load<RTexture>(
            "../Resources/Textures/Generic/DefaultAlbedo.png");
        m_emissionGeneric = TextureFactory::createOrGetTexture(DET);

        std::shared_ptr<RTexture> DGBT = m_resourceManager->load<RTexture>(
            "../Resources/Textures/Generic/GrayBoxTexture.png");
        m_emptyTextureGeneric = TextureFactory::createOrGetTexture(DGBT);

        std::shared_ptr<RShader> debugLineShaderVert = m_resourceManager->load<RShader>(
            "../Resources/Shaders/GLSL/Core/debugLine.vert");
        std::shared_ptr<RShader> debugLineShaderFrag = m_resourceManager->load<RShader>(
            "../Resources/Shaders/GLSL/Core/debugLine.frag");
