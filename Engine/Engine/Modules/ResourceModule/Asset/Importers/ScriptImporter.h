...
public:
                     ...
                     const std::filesystem::path& cacheRoot) override
    {
        ...
        if (!ifs.is_open())
            throw std::runtime_error("Cannot open Script file: " + sourcePath.string());

        std::string json((std::istreambuf_iterator<char>(ifs)), std::istreambuf_iterator<char>());
        ifs.close();
        
        LT_ASSERT_MSG(!json.empty(), "Script file is empty");
        LT_ASSERT_MSG(json.size() < 100 * 1024 * 1024, "Script JSON file is unreasonably large"); // 100MB limit

        std::filesystem::path outDir = cacheRoot / "Scripts";
        std::filesystem::create_directories(outDir);
        LT_ASSERT_MSG(std::filesystem::exists(outDir), "Failed to create Scripts directory");
        
        std::filesystem::path outFile = outDir / (sourcePath.stem().string() + ".Scriptbin");
        ...
        LT_ASSERT_MSG(size > 0, "Script size is zero");
        LT_ASSERT_MSG(size == json.size(), "Script size overflow");
        ...
        LT_LOGI("ScriptImporter", "Imported Script: " + sourcePath.string());
        return info;
    }
};
}